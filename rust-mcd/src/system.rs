...
use crate::{
    ...
    error::McdError,
    mcd_bindings::mcd_core_con_info_st,
    raw::{query_server::ServerConfig, McdReturnError},
    MCD_LIB,
};
...
impl System {
    ...
    pub(crate) fn connect(server_information: &ServerInfo) -> anyhow::Result<System> {
        ...
        let _ = MCD_LIB
            .open_server(configuration)
            .add_mcd_error_info(None)?;

        log::trace!("Scanning for systems attached to the server");

        let system_info = MCD_LIB.query_systems().add_mcd_error_info(None)?;
